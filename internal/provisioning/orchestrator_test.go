...
type opStoreFake struct {
	op provisioningdomain.TenantProvisioning
}

func (f *opStoreFake) GetTenantProvisioning(context.Context, string) (provisioningdomain.TenantProvisioning, error) {
	return f.op, nil
}
func (f *opStoreFake) UpdateTenantProvisioning(_ context.Context, n provisioningdomain.TenantProvisioning, expected int64) error {
	if f.op.Version != expected {
		panic("optimistic version mismatch")
	}
	f.op = n
	return nil
}
...
func (w workerFake) Run(context.Context, provisioningdomain.TenantProvisioning) (PhaseResult, error) {
...
func TestOrchestratorActivatesOnlyAfterConvergence(t *testing.T) {
	...
	if got.Status != provisioningdomain.ProvisioningStatusActive {
		t.Fatalf("got %s", got.Status)
	}
	if got.CompletedAt == nil {
		t.Fatal("ACTIVE must have completed_at")
	}
}
...
func TestOrchestratorStopsAtReconcileWhenDriftExists(t *testing.T) {
	...
	if got.Status != provisioningdomain.ProvisioningStatusReconcile {
		t.Fatalf("got %s", got.Status)
	}
}
