...
func (f resourceFake) ReconcileResource(context.Context, provisioningdomain.TenantProvisioning) ([]Drift, error) {
	return f.drift, nil
}
...
func TestReconciliationAdvancesObservedVersionOnlyOnFullConvergence(t *testing.T) {
	...
	if !report.Converged || report.ObservedStateVersion != 7 {
		t.Fatalf("%+v", report)
	}
}
...
func TestReconciliationPreservesObservedVersionOnDrift(t *testing.T) {
	...
	if report.Converged || report.ObservedStateVersion != 6 {
		t.Fatalf("%+v", report)
	}
}
