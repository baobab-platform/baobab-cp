...
func (f *grantStoreFake) GetCapability(context.Context, string) (capabilitydomain.Capability, error) { return f.capability, nil }
func (f *grantStoreFake) GetCapabilityScope(context.Context, string) (capabilitydomain.CapabilityScope, error) { return f.scope, nil }
func (f *grantStoreFake) ListGrants(context.Context, string, string) ([]capabilitydomain.CapabilityGrant, error) { return f.grants, nil }
func (f *grantStoreFake) CreateGrant(_ context.Context, g capabilitydomain.CapabilityGrant) error { f.grants = append(f.grants, g); return nil }
func (f *grantStoreFake) RevokeGrant(context.Context, string, string, string, int64) error { return nil }
...
func TestCapabilityGrantApplyIsIdempotent(t *testing.T) {
    ...
    if err != nil || !created { t.Fatalf("first apply: created=%v err=%v", created, err) }
    _, created, err = p.Apply(context.Background(), d)
    if err != nil || created { t.Fatalf("second apply must reuse existing: created=%v err=%v", created, err) }
    if len(f.grants) != 1 { t.Fatalf("expected one grant, got %d", len(f.grants)) }
}
...
func TestCapabilityGrantRejectsCrossTenantScope(t *testing.T) {
    ...
    if err == nil { t.Fatal("expected cross-tenant scope to fail") }
}
