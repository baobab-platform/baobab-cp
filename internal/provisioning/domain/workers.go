...
func (w ApplyWorker) Run(ctx context.Context, op provisioningdomain.TenantProvisioning) (PhaseResult, error) {
	if len(w.Steps)==0 { return PhaseResult{}, errors.New("APPLY has no configured materializers") }
	for _, step := range w.Steps {
		if step == nil { return PhaseResult{}, errors.New("nil APPLY step") }
		if err := step.Apply(ctx, op); err != nil { return PhaseResult{}, err }
	}
...
func (w ReconcileWorker) Run(ctx context.Context, op provisioningdomain.TenantProvisioning) (PhaseResult,error) {
	if w.Reconciler==nil { return PhaseResult{}, errors.New("reconciler is required") }
...
func (w ReadinessWorker) Run(ctx context.Context, op provisioningdomain.TenantProvisioning) (PhaseResult,error) {
	if w.Evaluator==nil { return PhaseResult{}, errors.New("readiness evaluator is required") }
