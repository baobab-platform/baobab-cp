...
func (f contextAuthorityFake) GetTenant(context.Context, string) (domain.Tenant, error) { return f.tenant, nil }
func (f contextAuthorityFake) GetMarket(context.Context, string) (domain.Market, error) { return f.market, nil }
func (f contextAuthorityFake) ListMarketAssignments(context.Context, string) ([]domain.MarketAssignment, error) {
	return []domain.MarketAssignment{f.assignment}, nil
}
func (f contextAuthorityFake) GetDigitalEstate(context.Context, string) (domain.DigitalEstate, error) { return f.estate, nil }
func (f contextAuthorityFake) GetIsolationProfile(context.Context, string) (domain.IsolationProfile, error) { return f.profile, nil }

func TestAuthoritativeContextResolution(t *testing.T) {
	now := time.Date(2026, 9, 17, 8, 0, 0, 0, time.UTC)
	f := contextAuthorityFake{
		tenant: domain.Tenant{ID: "tn_zuri", LegalEntityID: "ZURIBEANS"},
	...
	if got.ExpiresAt == nil { t.Fatal("expected bounded context lifetime") }
}
