...
func (m TenantManifest) Validate() error {
	if m.APIVersion != "baobab.nabhold.com/v1" {
		return errors.New("unsupported api_version")
	}
	if m.Kind != "TenantProvisioning" {
		return errors.New("kind must be TenantProvisioning")
	}
	...
	if m.Metadata.DesiredStateVersion < 1 {
		return errors.New("desired_state_version must be >= 1")
	}
	...
	for _, market := range m.Spec.Markets {
		...
		if _, exists := markets[code]; exists {
			return fmt.Errorf("duplicate market %s", code)
		}
		markets[code] = struct{}{}
	}
	for _, lane := range m.Spec.TradeLanes {
		if lane.OriginMarket == lane.DestinationMarket {
			return errors.New("trade lane origin and destination must differ")
		}
		if _, ok := markets[strings.ToUpper(lane.OriginMarket)]; !ok {
			return fmt.Errorf("unknown origin market %s", lane.OriginMarket)
		}
		if _, ok := markets[strings.ToUpper(lane.DestinationMarket)]; !ok {
			return fmt.Errorf("unknown destination market %s", lane.DestinationMarket)
		}
	}
	return nil
}
...
func (m TenantManifest) StableResourceKeys() []string {
	var keys []string
	for _, v := range m.Spec.Markets {
		keys = append(keys, "market:"+strings.ToUpper(v.MarketCode))
	}
	for _, v := range m.Spec.CapabilityGrants {
		keys = append(keys, "grant:"+v.CapabilityKey+":"+strings.ToUpper(v.MarketCode))
	}
	for _, v := range m.Spec.CapabilityBindings {
		keys = append(keys, "binding:"+v.CapabilityKey+":"+strings.ToUpper(v.MarketCode))
	}
	for _, v := range m.Spec.TradeLanes {
		keys = append(keys, "lane:"+strings.ToUpper(v.OriginMarket)+":"+strings.ToUpper(v.DestinationMarket))
	}
	sort.Strings(keys)
	return keys
}
