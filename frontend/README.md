...
| Package manager | pnpm, pinned by `packageManager` and enabled through corepack |
...
pnpm holds back newly published versions by its default minimum release age, and `pnpm-workspace.yaml` makes no
exceptions.
...
```sh
make frontend-install   # corepack enable, then pnpm install --frozen-lockfile
...
make frontend-image     # docker build of frontend/
```
...
`frontend/Dockerfile` builds with the Node 24 image and runs on distroless `nodejs24-debian13` as a non-root user.
...
`.github/workflows/frontend.yml` runs only when `frontend/` changes. It installs from the lockfile, then typechecks,
lints, tests, builds, audits production dependencies and builds the image.
