...
**Affected Repositories:** `baobab-platform/baobab-cp`, `baobab-platform/baobab-trade`, `baobab-platform/zuribeans`, `baobab-platform/shared`
...
- **Corrected by:** `shared` ADR-SHARED-008 — Gate ZB-01 Capability Domain Registration and Canonical Event-Type Convention Confirmation (registers the `market`/`internal-trade`/`customs`/`tax` domains this ADR's CR-006 relies on, and corrects this ADR's event-naming target from `baobab.*` to `com.nabhold.*`; see §2.1/§2.2 below)
...
Separately, canonical event naming was itself in dispute at the time this section was first drafted: this ADR originally treated `baobab.<bounded-context>.<aggregate>.<event>.v<major>` (per the Technical Specification's CR-003) as canonical and found ADR-BCP-011/013 conformant against it. **Correction (`shared` ADR-SHARED-008):** `baobab-platform/shared` — the actual contract authority — already ships and enforces `com.nabhold.<context>.<...>.v<N>` (`contracts/events/v1/envelope.schema.json`) across every real identity/ERP/supplier-onboarding event; the Technical Specification's CR-003 and this ADR's original item 5 below both specified the wrong format. See ADR-SHARED-008 §3 for the authoritative resolution. ADR-BCP-011 and ADR-BCP-013 are **not** conformant to the corrected convention either, despite matching the (incorrect) CR-003 format — all six of ADR-BCP-011/012/013/014 and `baobab-trade` ADR-0019/0020/0021/0022 need their event names expressed in `com.nabhold.*` form, not `baobab.*`.

### 2.2 Decision

1. **The masterplan §10 capability strings are canonical.** Where an ADR introduces a capability the masterplan already names, the ADR's local identifier is superseded by the §10 string. Where an ADR introduces a capability the masterplan does not name (e.g. `LEGAL_PRESENCE`, `PROCESSING`, `TRANSIT` market-participation states; `settlement.manage`; `transfer-price.approve`), the ADR's identifier is accepted into the canonical vocabulary and `baobab-platform/shared` SHALL add it to §10's registry on next revision, since these are legitimate gaps in the masterplan's enumeration rather than naming drift.
...
4. **`baobab-platform/shared` becomes the single point of registration.** Every capability ID used by any Baobab engine SHALL be published in Shared's capability taxonomy before a `CapabilityGrant` referencing it may be issued (this is already implied by ADR-BCP-003 and the Technical Specification §13; this ADR makes it explicit that the vocabulary conflicts above are what that registration step must resolve, not something CapabilityGrant issuance can paper over).
5. ~~All events raised by ADR-BCP-012, ADR-BCP-014, and `baobab-trade` ADR-0019/0020/0021/0022 SHALL be renamed to the canonical `baobab.<bounded-context>.<aggregate>.<event>.v<major>` form before those ADRs are accepted.~~ **Superseded by ADR-SHARED-008 §3.** The canonical form is `com.nabhold.<context>.<...>.v<N>`, per Shared's already-shipped and enforced `contracts/events/v1/envelope.schema.json`. Corrected example renamings: `supplier.registered` → `com.nabhold.procurement.supplier.registered.v1`; `commercial-price.resolved` → `com.nabhold.commercial.price.resolved.v1`; `shipment.created` → `com.nabhold.trade.shipment.created.v1`; `counterparty.merged` → `com.nabhold.counterparty.counterparty.merged.v1`. ADR-BCP-011 and ADR-BCP-013's existing event names (previously believed conformant) require the same correction.
...
- `baobab-platform/shared` owns producing the actual registry entries (capability taxonomy + AsyncAPI event definitions) reflecting the resolved names in the table above; no runtime repository resolves this itself.
...
- `baobab-platform/shared` carries an action item to publish the resolved capability taxonomy and event catalogue entries from §2.2 before Phase 1 (Technical Specification §88, "Programme Gate P1 — Shared Contract Foundation") schema work depending on them proceeds.
