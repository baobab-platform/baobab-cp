...
| **MarketParticipation** | `market.market_assignment` (`000006`) + `market.market_participation_capability` child table and `legal_entity_id` column (`000037`) | explicit footprint model: per-market `SOURCING`/`PROCUREMENT`/`SELLING`/`IMPORTING`/`EXPORTING`/`WAREHOUSING`/`DISTRIBUTION`/`FULFILMENT`/`LEGAL_PRESENCE`/`PROCESSING`/`TRANSIT` flags (ADR-BCP-011 §6/§53, corrected per ADR-BCP-015 CR-006) | **REMODEL — capability-flag dimension done** | `domain.MarketAssignment` now carries `Capabilities []MarketParticipationCapability` and optional `LegalEntityID`, so a tenant's market participation can represent *what* it may do there, not just that it participates. Still open, per ADR-BCP-011 §97's acceptance criteria: `status`/`source`/`policy_version` governance fields (`status` state machine per §45), `TradeLane` as a canonical resource, runtime resolution consuming lane/participation context, desired-state provisioning, drift reconciliation and readiness integration — all Programme Gate P6/P7/P10 scope, deliberately deferred from this capability-flag remodel. |
| **ProvisioningState** | `provisioning_operations` (`000019`, unchanged) + new `provisioning.tenant_provisioning` (`000038`) — `tenant_id`, `idempotency_key`/`request_hash` (its own UNIQUE-constrained idempotency guard), `status`, `desired_state_version`/`observed_state_version`, `product_requests[]`/`market_requests[]`, `blocking_reasons[]`, `attempt_count`/`last_error` | `TenantProvisioning` process aggregate (Technical Spec §21-22) | **ADD — basics done** | `provisioning.tenant_provisioning` is a new, separate table, not a remodel of `provisioning_operations` in place: `RegisterTenant` (`internal/store/postgres/store.go`) actively reads/writes `provisioning_operations` today as a live idempotency guard, so folding it into the new aggregate is left for a later gate, exactly as this row previously anticipated ("very likely become one of the inputs"). This basics cut models only Gate ZB-02's own simplified five-stage lifecycle (`PLAN`→`APPLY`→`RECONCILE`→`READY`→`ACTIVE`, plus `FAILED`/`CANCELLED` terminals with retry), not Technical Spec §22's full thirteen-state machine (`DRAFT`/`VALIDATING`/.../`VERIFYING_READINESS`/`REMEDIATING`/`DEPROVISIONED`) or any per-phase business logic (Context resolution, entitlement materialisation, provider binding) — Programme Gate P8 (Provider Reconciliation) onward. |
